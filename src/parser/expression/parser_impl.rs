...
use crate::{
    parser::Parser,
    types::{ParserError, TokenType},
};

macro_rules! new_ternary {
    ($eval:expr, $lhs:expr,  $rhs:expr) => {
        Expression::Ternary(Box::new(super::Ternary {
            evaluator: $eval,
            left: $lhs,
            right: $rhs,
        }))
...
macro_rules! new_assignment {
    ($name:expr, $value:expr) => {
        Expression::Assignment(Box::new(super::Assignment {
            name: $name,
            value: $value,
        }))
    };
...
macro_rules! new_expression {
    ($left:expr, $operator:expr,$right:expr) => {
        Expression::Binary(Box::new(super::Binary {
            operator: $operator,
            left: $left,
            right: $right,
        }))
    ...
    ($operator:expr, $operand:expr) => {
        Expression::Unary(Box::new(super::Unary {
            operator: $operator,
            operand: $operand,
        }))
    };
    ($expression:expr) => {
        Expression::Grouping(Box::new(super::Grouping {
...
macro_rules! new_literal {
    ($value:expr) => {
        Expression::Literal(Box::new(super::Literal { value: $value }))
    };
}
